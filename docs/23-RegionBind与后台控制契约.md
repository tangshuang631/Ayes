...
- 当存在当前任务时，应打开当前任务的 ROI 管理入口
- ROI 管理入口必须基于当前任务目标或最近截图进行框选，而不是要求用户口头输入像素坐标
- 用户可以多次添加 ROI、命名 ROI、禁用或删除 ROI
- 当前任务一旦设置启用 ROI，后续采样默认只处理启用 ROI；没有启用 ROI 时才回退为整目标采样
- ROI 增删必须通过服务/API 修改当前 `watch spec`，并写入审计日志

#### `open_settings`

- 打开交互式设置界面
- 设置界面至少能查看任务列表、近期日志、清理提醒间隔、本地视觉增强配置和熄屏整屏捕获策略
- 设置项必须通过服务/API 持久化，不能只存在于前端状态
...
- `process` / `window` 目标优先使用窗口捕获，理论上比整屏截图更适合显示器熄灭场景
- `screen` 目标在显示器熄灭时可能只能获得黑屏或不可读帧，取决于 macOS 和硬件是否仍暴露 framebuffer
- 设置项 `capture_screen_when_display_sleep` 表示“整屏监控在熄屏时尝试使用虚拟/离屏捕获策略”
- 若整屏熄屏捕获不可用，系统必须给出可理解提示，并建议用户改为监控进程或窗口
- 该设置只对 `screen` 目标有意义；`process` / `window` 目标不应因此改变默认窗口捕获策略

## 11. 第一阶段结论

- `region-bind` 必须成为独立正式合同，而不是散落在 skill 文案中的描述
- ROI 绑定、刷新点击点绑定和 `confirm-plan` 必须共享同一套稳定结构
- 桌面小图标菜单必须成为后台运行时的人类控制入口，并能进入 ROI 管理和设置界面
