...
class FailFirstWindowCapture(FakeCapture):
    def capture_window(self, candidate: WindowCandidate, *, timestamp: float) -> CaptureResult:
        if candidate.window_id == 201:
            self.calls += 1
            self.window_calls.append(candidate.window_id)
            return CaptureResult(ok=False, status="no_pixels", message="窗口 201 无法生成图像")
        return super().capture_window(candidate, timestamp=timestamp)


class FailAllWindowCapture(FakeCapture):
    def capture_window(self, candidate: WindowCandidate, *, timestamp: float) -> CaptureResult:
        self.calls += 1
        self.window_calls.append(candidate.window_id)
        return CaptureResult(ok=False, status="no_pixels", message=f"窗口 {candidate.window_id} 无法生成图像")
...
class DisplayCropCapture(FailAllWindowCapture):
    def __init__(self) -> None:
        super().__init__()
        self.main_display_calls = 0
    ...
    def capture_main_display(self, *, timestamp: float) -> CaptureResult:
        ...
        return CaptureResult(
            ...
            status="ok",
            frame=CaptureFrame(
                frame_id="display_crop_main",
                timestamp=timestamp,
                target_type="screen",
                target_id="main",
                width=1600,
                height=1200,
                image_bytes=make_sized_png_bytes(1600, 1200, color="green"),
            ),
        )
...
class FakeDiscovery:
    ...
    def list_windows_for_process(self, *, process_name=None, process_id=None, only_observable=True):
        if self.process_window is None:
            return []
        if only_observable and not self.process_window.observability.is_recommended:
            return []
        return [self.process_window]
    ...
    def get_frontmost_process_name(self):
        return ""
...
class MultiCandidateDiscovery:
    def __init__(self, windows) -> None:
        self.windows = list(windows)
        self.calls = []
        self.frontmost_name = self.windows[0].process_name if self.windows else ""

    def get_primary_window_for_process(self, *, process_name=None, process_id=None, only_observable=True):
        self.calls.append(("primary", only_observable))
        filtered = [item for item in self.windows if (not only_observable) or item.observability.is_recommended]
        return filtered[0] if filtered else None

    def list_windows_for_process(self, *, process_name=None, process_id=None, only_observable=True):
        self.calls.append(("list", only_observable))
        return [item for item in self.windows if (not only_observable) or item.observability.is_recommended]

    def activate_process(self, *, process_name=None, process_id=None):
        self.calls.append(("activate", process_name or process_id))
        if self.windows:
            self.frontmost_name = self.windows[0].process_name
        return True
...
class RefreshingDiscovery:
    def __init__(self, first_windows, second_windows) -> None:
        self.first_windows = list(first_windows)
        self.second_windows = list(second_windows)
        self.calls = []
        self.frontmost_name = "WPS"
        self._activated = False

    def list_windows_for_process(self, *, process_name=None, process_id=None, only_observable=True):
        self.calls.append(("list", only_observable, self._activated))
        source = self.second_windows if self._activated else self.first_windows
        return [item for item in source if (not only_observable) or item.observability.is_recommended]

    def get_primary_window_for_process(self, *, process_name=None, process_id=None, only_observable=True):
        items = self.list_windows_for_process(
...
def test_process_capture_uses_discovery_alias_match() -> None:
    spec = WatchSpec.from_dict(
        {
            "spec_version": "1.0",
            "mode": "observe",
            "target": {"type": "process", "process_name": "微信"},
            "watch_intent": {"enabled": False},
        }
    )
    capture = FakeCapture()
    discovery = FakeDiscovery(process_window=make_window_candidate(window_id=88, process_name="WeChat"))
    runner = WatchRunner(spec, task_id="task_wechat")
    ...
    assert text_event.target.window_id == 88
    assert capture.window_calls == [88]
...
class RecordingOCR:
    def __init__(self, text_by_region=None) -> None:
        self.calls = []
        self.text_by_region = text_by_region or {}

    def recognize(self, image, options=None) -> OCRResult:
        region_id = getattr(image, "region_id", "") or ""
        self.calls.append({"region_id": region_id, "options": dict(options or {})})
        text = self.text_by_region.get(region_id, "普通内容")
        return OCRResult(
            provider="fake",
            elapsed_ms=1,
            full_text=text,
            char_count=len(text),
            blocks=[OCRTextBlock(text=text, confidence=0.91)],
        )
...
def test_runner_process_target_falls_back_to_partial_window_when_observable_missing() -> None:
    class FallbackDiscovery:
        def __init__(self) -> None:
            self.calls = []

        def get_primary_window_for_process(self, *, process_name=None, process_id=None, only_observable=True):
            self.calls.append(only_observable)
            if only_observable:
                return None
            candidate = make_window_candidate(window_id=66, process_name="WeChat")
            return WindowCandidate(
        ...
        def list_windows_for_process(self, *, process_name=None, process_id=None, only_observable=True):
            candidate = self.get_primary_window_for_process(
                process_name=process_name,
                process_id=process_id,
                only_observable=only_observable,
            )
    ...
    spec = WatchSpec.from_dict(
        {
            "spec_version": "1.0",
            "mode": "observe",
            "target": {"type": "process", "process_name": "微信", "only_observable_windows": True},
            "sampling": {
                "screenshot_interval_ms": 1,
                "ocr_interval_ms": 1,
                "change_detection_interval_ms": 1,
    ...
    runner = WatchRunner(spec)
    runner.capture = FakeCapture()
    runner.discovery = FallbackDiscovery()
    runner.ocr = FakeOCR()

    events = runner.run_once(now=100.0)

    assert runner.discovery.calls == [True, False]
    text_event = next(event for event in events if event.event_type == "text_change")
    assert text_event.target.window_id == 66
    assert runner.capture.window_calls == [66]


def test_runner_process_target_retries_next_candidate_after_no_pixels() -> None:
    first_candidate = make_window_candidate(window_id=201, process_name="WeChat")
    second_candidate = WindowCandidate(
        window_id=202,
        process_id=100,
        process_name="WeChat",
        title="微信会话",
        bounds=Bounds(x=0, y=0, width=880, height=640),
        layer=0,
        is_onscreen=False,
        observability=ObservabilityStatus(code="partial", label="部分可观测", has_pixels=True, is_recommended=False),
        is_business_candidate=True,
    ...
    runner = WatchRunner(spec)
    runner.capture = FailFirstWindowCapture()
    runner.discovery = MultiCandidateDiscovery([first_candidate, second_candidate])
    runner.ocr = FakeOCR()

    events = runner.run_once(now=100.0)

    text_event = next(event for event in events if event.event_type == "text_change")
    assert text_event.target.window_id == 202
    assert runner.capture.window_calls == [201, 202]


def test_runner_process_target_can_fallback_to_display_crop_when_frontmost_matches() -> None:
    main_candidate = WindowCandidate(
        window_id=201,
        process_id=100,
        process_name="WeChat",
        title="微信主窗口",
        bounds=Bounds(x=10, y=10, width=880, height=640),
        layer=0,
        is_onscreen=False,
        observability=ObservabilityStatus(code="partial", label="部分可观测", has_pixels=True, is_recommended=False),
        is_business_candidate=True,
    ...
    runner = WatchRunner(spec)
    runner.capture = DisplayCropCapture()
    runner.discovery = MultiCandidateDiscovery([main_candidate])
    runner.discovery.frontmost_name = "WPS"
    runner.ocr = FakeOCR()

    events = runner.run_once(now=100.0)

    text_event = next(event for event in events if event.event_type == "text_change")
    assert text_event.target.window_id == 201
    assert runner.capture.window_calls == [201, 201]
    assert runner.capture.main_display_calls == 1
    assert ("activate", "微信") in runner.discovery.calls


def test_runner_process_target_reenumerates_after_activation_before_display_crop() -> None:
    first_candidate = WindowCandidate(
        window_id=201,
        process_id=100,
        process_name="WeChat",
        title="微信空窗口",
    ...
    runner = WatchRunner(spec)
    runner.capture = FailFirstWindowCapture()
    runner.discovery = RefreshingDiscovery([first_candidate], [second_candidate])
    runner.ocr = FakeOCR()
...
def test_runner_uses_quality_ocr_selection_for_primary_attention_region_only() -> None:
    spec = WatchSpec.from_dict(
        {
            "spec_version": "1.0",
            "mode": "observe",
    ...
    runner = WatchRunner(spec)
    runner.capture = LargeFakeCapture()
    runner.ocr = RecordingOCR()

    runner.run_once(now=100.0)

    primary_call = next(call for call in runner.ocr.calls if call["region_id"] == "auto_center_main")
    edge_call = next(call for call in runner.ocr.calls if call["region_id"] == "auto_right_panel")
    assert primary_call["options"]["selection_mode"] == "quality"
    assert edge_call["options"] == {}


def test_runner_marks_edge_abnormal_keywords_as_boosted_context() -> None:
    spec = WatchSpec.from_dict(
        {
            "spec_version": "1.0",
            "mode": "observe",
    ...
    runner = WatchRunner(spec)
    runner.capture = LargeFakeCapture()
    runner.ocr = RecordingOCR({"auto_right_panel": "支付失败，请重新登录"})

    events = runner.run_once(now=100.0)

    right_panel = next(event for event in events if event.event_type == "text_change" and event.region.region_id == "auto_right_panel")
    attention = right_panel.visual.attributes["attention"]
    assert attention["abnormal_keyword_boosted"] is True
    assert attention["weight"] > 0.32
