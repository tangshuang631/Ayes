...
def test_control_settings_round_trip_monitor_context_prompt() -> None:
    response = client.post(
        "/api/control/settings",
        json={"monitor_context_prompt": "Ayes context mode"},
    )
...
def test_control_settings_ignores_task_hotkey_fields() -> None:
    response = client.post(
        "/api/control/settings",
        json={"latest_frame_hotkey": "cmd+shift+9", "monitor_context_hotkey": "cmd+shift+8"},
    )

    assert response.status_code == 200
    payload = response.json()
    assert "latest_frame_hotkey" not in payload["settings"]
    assert "monitor_context_hotkey" not in payload["settings"]


def test_task_hotkey_policy_round_trip_and_clear() -> None:
    task_id = f"task_hotkey_policy_round_trip_{uuid4().hex}"
    client.post(
        f"/api/tasks/{task_id}/hotkey-policy",
        json={"latest_frame_hotkey": "", "monitor_context_hotkey": ""},
    )
    client.post(
        "/api/watch/load-configured",
        json={
    ...
    response = client.post(
        f"/api/tasks/{task_id}/hotkey-policy",
        json={"latest_frame_hotkey": "cmd+ctrl+f18", "monitor_context_hotkey": "cmd+option+f19"},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["hotkeys"]["latest_frame_hotkey"] == "cmd+ctrl+f18"
    assert payload["hotkeys"]["monitor_context_hotkey"] == "cmd+option+f19"
    assert payload["warnings"]["latest_frame_hotkey"] == []
    assert payload["warnings"]["monitor_context_hotkey"] == []
    ...
    assert cleared.json()["hotkeys"]["latest_frame_hotkey"] == ""
    assert cleared.json()["hotkeys"]["monitor_context_hotkey"] == ""


def test_task_hotkey_policy_warns_on_dangerous_and_duplicate_shortcuts() -> None:
    suffix = uuid4().hex
    first_task_id = f"task_hotkey_policy_warn_first_{suffix}"
    second_task_id = f"task_hotkey_policy_warn_second_{suffix}"
    ...
    first_response = client.post(
        f"/api/tasks/{first_task_id}/hotkey-policy",
        json={"latest_frame_hotkey": "cmd+v", "monitor_context_hotkey": "cmd+shift+8"},
    )
    assert first_response.status_code == 200
    first_payload = first_response.json()
    assert any(item["code"] == "system_common" for item in first_payload["warnings"]["latest_frame_hotkey"])

    second_response = client.post(
        f"/api/tasks/{second_task_id}/hotkey-policy",
        json={"latest_frame_hotkey": "cmd+shift+8", "monitor_context_hotkey": "cmd+shift+8"},
    )
    assert second_response.status_code == 200
    second_payload = second_response.json()
    assert any(item["code"] == "duplicate_other_task" for item in second_payload["warnings"]["latest_frame_hotkey"])
    assert any(item["code"] == "duplicate_same_task" for item in second_payload["warnings"]["monitor_context_hotkey"])
