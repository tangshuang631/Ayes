...
def test_agent_contracts_endpoint_exposes_expected_routes() -> None:
    ...
    assert payload["tasks.hotkey_policy"]["path"] == "/api/tasks/{task_id}/hotkey-policy"
    assert "latest_frame_hotkey" in payload["tasks.hotkey_policy"]["request"]
    assert "warnings" in payload["tasks.hotkey_policy"]["response_keys"]
