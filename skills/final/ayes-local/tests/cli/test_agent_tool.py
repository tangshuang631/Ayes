...
def test_agent_tool_process_candidates_calls_task_endpoint(monkeypatch, capsys) -> None:
    recorded = {}
    ...
    exit_code = agent_tool.main(["process-candidates", "--task-id", "task_wechat", "--auto-focus"])

    assert exit_code == 0
    assert recorded["path"] == "/api/tasks/task_wechat/process-candidates?auto_focus=True"


def test_agent_tool_rebind_process_calls_task_endpoint(monkeypatch, capsys) -> None:
    recorded = {}
    ...
    exit_code = agent_tool.main(
        [
            ...
            "--window-id",
            "201",
            "--only-observable-windows",
            "false",
        ]
    )

    assert exit_code == 0
    assert recorded["path"] == "/api/tasks/task_wechat/rebind-process"
    assert recorded["method"] == "POST"
    assert recorded["payload"]["window_id"] == 201
    assert recorded["payload"]["process_name"] == "WeChat"


def test_agent_tool_rename_task_calls_task_endpoint(monkeypatch, capsys) -> None:
    recorded = {}
    ...
    exit_code = agent_tool.main(["rename-task", "--task-id", "task_wechat", "--display-name", "微信"])

    assert exit_code == 0
    assert recorded["path"] == "/api/tasks/task_wechat/rename"
    assert recorded["method"] == "POST"
    assert recorded["payload"] == {"display_name": "微信"}
...
def test_agent_tool_run_once_uses_extended_timeout(monkeypatch, capsys) -> None:
    recorded = {}

    def fake_request_json(base_url, path, *, method="GET", payload=None, timeout_sec=10):
        recorded["base_url"] = base_url
        recorded["path"] = path
        recorded["method"] = method
        recorded["payload"] = payload
    ...
    assert recorded["timeout_sec"] == 60
...
def test_build_menubar_settings_includes_ollama_help(tmp_path: Path) -> None:
    app_path = agent_tool.build_menubar_app_bundle(
        root_dir=tmp_path,
        runtime_dir=tmp_path / "runtime",
        python_bin="/usr/bin/python3",
    ...
    assert "https://ollama.com" in source
    assert "ollama pull qwen2.5vl:7b" in source
    assert "启用 Ayes 本地模型增强" in source
...
def test_build_menubar_native_host_formats_task_target_titles(tmp_path: Path) -> None:
    ...
    assert '[task objectForKey:@"display_name"]' in source
    assert '@"绑定进程..."' in source
...
def test_build_menubar_settings_uses_hotkey_recording_mode(tmp_path: Path) -> None:
    app_path = agent_tool.build_menubar_app_bundle(
        root_dir=tmp_path,
        runtime_dir=tmp_path / "runtime",
        python_bin="/usr/bin/python3",
    ...
    source = (app_path / "Contents" / "MacOS" / "AyesMenubar.m").read_text(encoding="utf-8")
    assert "recordedHotkeyFromEvent" in source
    assert "beginHotkeyCaptureForFieldName" in source
    assert "hotkeyFieldClicked" in source
    assert "请按快捷键，Esc 清空" in source
    assert "录制" not in source


def test_build_menubar_only_task_settings_show_hotkeys(tmp_path: Path) -> None:
    app_path = agent_tool.build_menubar_app_bundle(
        root_dir=tmp_path,
        runtime_dir=tmp_path / "runtime",
        python_bin="/usr/bin/python3",
    ...
    source = (app_path / "Contents" / "MacOS" / "AyesMenubar.m").read_text(encoding="utf-8")
    assert "if (isTaskSpecificSettings)" in source
    assert "截图快捷键" in source
    assert "提问快捷键" in source
    assert '@"/api/tasks/" stringByAppendingFormat:@"%@/hotkey-policy", encoded' in source
    assert "[self refreshStatus:nil];" in source


def test_build_menubar_task_settings_persist_task_vision_config(tmp_path: Path) -> None:
    app_path = agent_tool.build_menubar_app_bundle(
        root_dir=tmp_path,
        runtime_dir=tmp_path / "runtime",
        python_bin="/usr/bin/python3",
    ...
    source = (app_path / "Contents" / "MacOS" / "AyesMenubar.m").read_text(encoding="utf-8")
    assert '@"/api/tasks/" stringByAppendingFormat:@"%@/vision-policy", encoded' in source
    assert 'if (isTaskSpecificSettings)' in source
...
def test_build_menubar_global_settings_does_not_target_current_task_sampling(tmp_path: Path) -> None:
    ...
    assert "当前为全局设置；不会覆盖任务专属采样、记忆和快捷键配置。" in source
