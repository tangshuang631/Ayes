...
def test_final_skill_doc_prioritizes_wrapper_plus_query_without_path_hunting() -> None:
    skill_doc = Path("/Users/apple/Desktop/2026/Ayes/skills/final/ayes-local/SKILL.md").read_text(encoding="utf-8")

    assert "$HOME/.codex/skills/ayes-local/scripts/ayes-agent-local query --minutes 240" in skill_doc
    assert "Do not spend time searching `PATH` first." in skill_doc
    assert "Do not start with `status`, `ensure-service`, and PATH hunting" in skill_doc
