...
def build_install_paths(*, repo_root: Path, skill_root: Path) -> InstallPaths:
    ...
    return InstallPaths(
        ...
        source_skill_dir=resolved_repo_root / "skills" / "ayes-local",
        target_skill_dir=target_skill_dir,
        target_scripts_dir=target_scripts_dir,
        wrapper_path=target_scripts_dir / "ayes-agent-local",
    )
...
def install_skill(*, repo_root: Path, skill_root: Path, python_bin: str = "python3", module: str = "ayes.cli.agent_tool") -> InstallPaths:
    ...
    wrapper = build_wrapper_script(repo_root=paths.repo_root, python_bin=python_bin, module=module)
    paths.wrapper_path.write_text(wrapper, encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"本地包装命令: {paths.wrapper_path}")
    print("安装后建议验证：")
    print(f"  {paths.wrapper_path} ensure-service")
    print(f"  {paths.wrapper_path} contracts")
    print(f"  {paths.wrapper_path} status")
    return 0
